...
URLS = {
    "translation": {
        "url": "http://www.acarsd.org/download/translation.php",
        "md5": None,
...
def update_translation():
    ...
    # Remove old datas
    db.session.query(Translation).filter(
        or_(Translation.source == "", Translation.source == "translation.csv")
    ).delete()
    db.session.commit()
...
def update_mode_s():
    ...
    db.session.query(AircraftModes).filter(
        or_(AircraftModes.source == fname, AircraftModes.source.is_(None), AircraftModes.source == "")
    ).delete()
    db.session.query(AircraftOwner).filter(
        or_(AircraftOwner.source == fname, AircraftOwner.source.is_(None), AircraftOwner.source == "")
    ).delete()
    db.session.commit()
    ...
    with open(csv_filename, "rt") as csv_file:
        for row in csv.reader(csv_file, delimiter=",", quotechar="'"):
            ...
            if len(aircraft_name_split) > 1 and len(aircraft_name_split[1]) > 3:
                q = Aircrafts.query.filter(
                    and_(Aircrafts.type.like(aircraft_name), Aircrafts.type.like(aircraft_name_split[0]))
                )
